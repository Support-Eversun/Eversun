...
export default function ClientCalendar({ section, items, onEdit }: ClientCalendarProps) {
  ...
  const getUrgencyInfo = (client: ClientRecord) => {
    ...
    if (diffDays < 0) {
      return { color: 'from-red-500 to-rose-600', label: 'En retard', bg: 'bg-red-50 dark:bg-red-900/20 border-red-500', urgent: true, diffDays };
    }
    if (diffDays === 0) {
      return { color: 'from-red-500 to-orange-500', label: "Aujourd'hui", bg: 'bg-orange-50 dark:bg-orange-900/20 border-orange-500', urgent: true, diffDays };
    }
    if (diffDays <= 3) {
      return { color: 'from-orange-500 to-amber-500', label: 'Urgent', bg: 'bg-amber-50 dark:bg-amber-900/20 border-amber-500', urgent: true, diffDays };
    }
    if (diffDays <= 7) {
      return { color: 'from-yellow-500 to-amber-500', label: 'Proche', bg: 'bg-yellow-50 dark:bg-yellow-900/20 border-yellow-500', urgent: false, diffDays };
    }
    if (diffDays <= 14) {
  ...
  const upcomingNonUrgent = upcomingEvents.filter((item) => !item.urgency?.urgent);
